; (function () {
  ...
  $('.results-filters button').click(function () {
    $('.results-filters .filter-header img').toggleClass('open');
    $('.results-filters .filter-header').toggleClass('open');
  });
  ...
  var determineCollapse = function () {
  ...
  var renderResults = function (teaserData, queryData) {
    if (teaserData) {
      ...
      if (teaserData.recordCount === 0) { //coerce
        ...
        if (typeof queryData.bvid !== 'undefined') {
          var resultId = _.findIndex(teaserData.teasers, function (t) { return t.bvid == queryData.bvid; });
          if (typeof resultId !== 'undefined' && _.isFinite(resultId)) {
  ...
  var getTeaserData = function (data, initiator) {
    ...
    var url = baseUrl + "&fn=" + data.fn + "&ln=" + data.ln + "&state=" + validState(data.state) + "&city=" + validData(data.city) + "&age=" + validData(data.age) + "&mn=" + validData(data.mi);
    var xhrData = $.ajax({
      url: url,
      dataType: 'jsonp',
    ...
    $.when(xhrData).done(function (result) {
      ...
      renderResults(teaserDataObj, data);
  ...
  $(".bv-search").on('submit', function (evt) {
    ...
    $toggleSearchBar.css({ "display": "none" });
  });
  ...
  var exitPop = function () {
    ...
    // Show the modal.
    $('#refine-modal').modal('show');
    $('#refine-modal').find('input[name="fn"]').focus();

    $('#refine-modal .modal-dialog').on('click', function (e) {
...
(function () {
  ...
  $toggleSearchBar = $('.search-toggled');

  $('.btn-search-bar').click(function () {
    if ($toggleSearchBar.hasClass('visible')) {
      $toggleSearchBar.css({ "display": "none" });
      $toggleSearchBar.removeClass('visible');
    } else {
      $toggleSearchBar.addClass('visible');
      $toggleSearchBar.css({ "display": "block" });
    }
  });
  ...
  /* ====== Replace Abreviated State to Full State ====== */
  var stateConverted = false;

  var callToConvertStates = function () {
    $(".fullState").each(function (i, obj) {
      var abrevState = obj.innerText;
      var fullState = convert_state(abrevState, "name");
  ...
  .on('gettingResults', function () {
    stateConverted = false;
  })
  .on('newResults', function () {
    updateStates();
  });
