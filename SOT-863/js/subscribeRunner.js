// NOTE: This has been updated for NumberGuru

// Reverse phone subscribe.js
(function () {

  var url = 'https://numberguru.com/hk/dd/free/phoneinfo?&exporttype=jsonp';
  var subCheckUrl = 'https://numberguru.com/hk/dd/premium/phone/reverseteaser?exporttype=jsonp';
  var mapNum = 60352;
  var apiKey = 'cc179a9d62854dadb9147401e2f1c942';
  var tilesUrl = 'https://ssl_tiles.cloudmade.com/' + apiKey + '/' + mapNum + '/256/{z}/{x}/{y}.png';
  var landingUrl = "https://www.numberguru.com/reverse-phone-landing";
  var subscribeUrl = "https://www.numberguru.com/subscribe/reverse_phone_sub2_fixes";
  var blueCheckUrl = '//manaron.s3.amazonaws.com/srg/Reverse-Phone/landing/img/blueCheck_32.png';
...
window._trackmetrics = window._trackmetrics || {};

(function ($, amplify, metrics) {

  window.isDataAvailable = true;

  //$('input, textarea').placeholder();

  var ACTIVATE_YOUR_ACCOUNT= "Activate Account To Search";


  var report = function (methodName, opt) {
    if (metrics && metrics[methodName]) {
      metrics[methodName](opt);
    }
  };

  var searchData = amplify.store(bvRPL.config.searchData),
      sendDataTo = amplify.store(bvRPL.config.sendDataTo);

  // console.log('THE SEARCH DATA IS ', searchData, sendDataTo);

  var fillSearchData = function () {
    $('.phone-number').html(searchData.formattedPhoneNumber);
    ...
    // Dirty patching to get rid of activate links from previous page.
    searchData.data.ownersName = ACTIVATE_YOUR_ACCOUNT;
    searchData.data.zipCode = ACTIVATE_YOUR_ACCOUNT;
    searchData.data.streetAddress = ACTIVATE_YOUR_ACCOUNT;
    searchData.data.neighborhood = ACTIVATE_YOUR_ACCOUNT;
    searchData.data.elevation = ACTIVATE_YOUR_ACCOUNT;
    searchData.data.latitude = ACTIVATE_YOUR_ACCOUNT;

    $.each(searchData.data, function (key, val) {
      if (typeof $elems[key] !== 'undefined') {
        $elems[key].html(val);
      }
    });
  ...
  var fillFormData = function () {
    $.each(sendDataTo, function (idx, obj) {
      if (obj.name === "account[first_name]") {
        $("#account_first_name").val(obj.value);
      } else if (obj.name === "account[last_name]") {
        $("#account_last_name").val(obj.value);
      } else if (obj.name === "user[email]") {
        $("#user_email").val(obj.value);
      }
    });
  };
  ...
  var createMap = function () {
    ...
    map.setView(searchData.latlng, 13);
  };



  var initializeRPL = function () {
    ...
    var subCheckUrl = bvRPL.config.subCheckUrl + "&phone=" + searchData.phoneNumber,
    availabilityCheck = $.ajax(subCheckUrl, {dataType: 'jsonp'});

    $.when(availabilityCheck).then(function (response, state) {
      if (state === "success") {
        isDataAvailable = response.results && !$.isEmptyObject(response.results.type);

        var resultType = "";

        if (response.results && response.results.type) {
          resultType = (response.results.type + "").toLowerCase();
        }

        if (resultType == "b") {
          report('businessNumberSearched', {"RPL PhoneNumber": searchData.phoneNumber});
        } else if (resultType == "c") {
          report('consumerNumberSearched', {"RPL PhoneNumber": searchData.phoneNumber});
        }

        if (resultType && resultType == "u") {
          isDataAvailable = false;
        }

        if (isDataAvailable) {
          report('numberFound', {"RPL PhoneNumber": searchData.phoneNumber});
        } else {
          report('numberNotFound', {"RPL PhoneNumber": searchData.phoneNumber});
        }
      }
    });
...
;(function (w, $) {
  var VERIFI_ENDPOINT = w.BV_VERIFI_ENDPOINT || 'https://secure.verifi.com/api/v2/three-step/',
      ...
      REDIRECT_IDENTIFIER = 'numberguru.com';
