(function ($, undefined) {
  ...
  var modals = [{
    ...
    transitionDelay: 500,         // After progress completion, amount of time before moving to next flow.
    animate: function () {
  ...
  function initializingSearchProgress() {
    ...
    var $progressBars = [
      ...
      $('#searching-progress-bar-datasets .knob-circle')
    ];
    ...
    _.forEach($progressBars, function ($elem, idx) {
      ...
      var duration = splits[idx],
          animation = $({value: 0}).animate({ value: 100 }, {
            duration: duration,
            easing: 'swing',
            progress: function () {
              $elem.val(Math.ceil(this.value)).trigger('change')
            }
          });
