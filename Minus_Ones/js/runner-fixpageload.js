(function() {
	...
	var initAddress = function() {
		...
		var liveaddress = $.LiveAddress({
			...
			invalidMessage:
				"We did not find that address in our records<br><span class='line_two'>Be sure to include a building number and leave out resident names</span>",
			stateFilter:
		...
		liveaddress.on('InvalidAddressRejected', function(
			event,
			data,
			previousHandler
		) {
	...
	if ($headerSearchPhone.length !== 0) {
		...
		$headerSearchPhone.validate({
			...
			submitHandler: function(form, event) {
				...
				var encoded_phone = encodeURIComponent(base64.encode(cleanNumber));

				window.setTimeout(function() {
					var nextPage =
						$('#header-search-phone').attr('action') ||
						'https://www.beenverified.com';

					window.location = nextPage + '?' + 'phone=' + encoded_phone;
				}, REQUEST_DELAY);
			}
		});
	}
	if ($headerSearchEmail.length !== 0) {
		$headerSearchEmail.validate({
			...
			submitHandler: function(form, event) {
				...
				var urlArray = $('#header-search-email').serializeArray();
				var encoded_email = encodeURIComponent(
					base64.encode(urlArray[0].value)
				);

				window.setTimeout(function() {
					var nextPage =
						$('#header-search-email').attr('action') ||
						'https://www.beenverified.com';

					window.location = nextPage + '?' + 'emailaddress=' + encoded_email;
				}, REQUEST_DELAY);
			}
		});
	}
