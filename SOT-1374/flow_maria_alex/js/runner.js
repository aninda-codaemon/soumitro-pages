  (function($) {
      ...
      var cleanQueryData = function(data) {
          ...
          data = parseMiddleInitial(_.mapValues(data, cleanSearchValues));
      ...
      var progress = $bar.animate({
          ...
          step: function(step) {
              ...
              if (percent === 42) {
                  $('.test1').addClass('hidden-xs');
                  $('.test2').removeClass('hidden-xs');
              }
              if (percent === 66) {
                  $('.test2').addClass('hidden-xs');
                  $('.test3').removeClass('hidden-xs');
              }
          }
      });
      ...
      var buildXhrUrl = function() {
          var baseUrl = "https://www.beenverified.com/hk/teaser/?exporttype=jsonp&rc=100";
      ...
      var getResultCount = function() {
          ...
          $.when(xhrData).then(function(result) {
              ...
              if (status === "0") {
                  ...
                  trackNL("Refine Modal Initial Result Count", {
      ...
      $.when(progress).done(function() {
          ...
          $.when(xhrData).then(function(result) {
              ...
              if (status === "0") {
                  ...
                  window.recordsTotal = recordCount;

                  trackNL("Refine Modal Final Result Count", {
                      result_count: recordCount
                  });
  ...
  (function() {
      ...
      setInterval(function() {
          hideQuestion();
      }, 14000); // <-- time in milliseconds

      var firstRun = true;
      var running = true;
      var counter = 0,
          divs = $('#quest1, #quest2, #quest3, #quest4');
      ...
      function hideQuestion() {
          ...
          if (running) {
              ...
              }, 13 * 1000); // do this every 10 seconds
              running = false;
          }

      }

      if (amplify.store('teaserData')) {
        recordsCount = '';
      }

      function firstCountTimer() {
          countDown(40000, '1000000000', '10000');
      }

      function secCountTimer(records) {
          if (firstRun) {
              countDown(20000, '10000', records);
              firstRun = false;
          }
      }


      function countDown(dur, fromVal, toVal) {

          $('.count').each(function() {
              $(this).prop('Counter', fromVal).animate({
                  Counter: toVal
              }, {
                  duration: dur,
                  easing: 'easeBV',
                  step: function(now) {
                      ...
                      var tempNum = commafy(now.toString());
                  ...
                  complete: function() {
                      ...
                      if (window.recordsTotal) {
                        recordsCount = window.recordsTotal.toString();
                      }

                      secCountTimer(recordsCount);
                  }
      ...
      function showDiv() {
          divs.fadeOut('fast') // hide all divs
