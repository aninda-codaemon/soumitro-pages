  (function ($) {
    ...
    var progress = $bar.animate({
        ...
        step: function(step) {
          ...
          if (percent === 18) {
            $('.loading-detail#loadingStep1').addClass('hidden');
            $('.loading-detail#loadingStep2').removeClass('hidden');
          }
          if (percent === 34) {
            $('.loading-detail#loadingStep2').addClass('hidden');
            $('.loading-detail#loadingStep3').removeClass('hidden');
          }
          if (percent === 42) {
            $('.testimonial#natasha').addClass('hidden');
            $('.testimonial#jr').removeClass('hidden');
          }
          if (percent === 54) {
            $('.loading-detail#loadingStep3').addClass('hidden');
            $('.loading-detail#loadingStep4').removeClass('hidden');

            $('.progress-text-inner').css('color', '#fff');
          }
          if (percent === 66) {
            $('.loading-detail#loadingStep4').addClass('hidden');
            $('.loading-detail#loadingStep5').removeClass('hidden');

            $('.testimonial#jr').addClass('hidden');
            $('.testimonial#rhonda').removeClass('hidden');
          }
          if (percent === 85) {
            $('.loading-detail#loadingStep5').addClass('hidden');
            $('.loading-detail#loadingStep6').removeClass('hidden');

            $('#no-patience').css('display', 'block');
          }
          if (percent === 98) {
    ...
    if (_.isEmpty(searchData) || !validSearchData(searchData)) {
      ...
      $('.header .headline').html('Gathering data from our servers');
    ...
    var getResultCount = function() {
      ...
      $.when(xhrData).then(function(result){
        ...
        if (recordCount > 10) {
          ...
          $('.results-found-count').html(recordCount);
        } else {
          if (recordCount === 0) {
            $(".results-loaded-statement").hide();
          } else {
  ...
  (function () {
    ...
    // initiate downsell after 5 seconds
    setTimeout(function() {
      downsell.init({
        onBack: {
          elem: "#iModal-wait",
    ...
    // trigger secondary downsell modal
    $('.close-modal-alt').click(function(evt) {
      evt.preventDefault();

      $('#iModal-wait').modal('hide');

      if ($('.progress-text-inner').text().slice(0, 2) > 85) {
        $('#iModal-inspire').modal('show');
      }
    });
  }());
